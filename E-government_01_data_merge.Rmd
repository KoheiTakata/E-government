...
```{r temp_1, include=TRUE}

## reproduction of party strength index
temp <-data_PS %>% 
       filter(year > 1999) %>% 
       mutate(v2psswitch_neg_std = (-1*v2psswitch - mean(-1*v2psswitch))/sd(-1*v2psswitch),
              v2pscohesv_std     = (   v2pscohesv - mean(v2pscohesv))   /sd( 　v2pscohesv),
              ...
              ps_new = (v2psswitch_neg_std +v2pscohesv_std + v2pscnslnl_neg_std + v2psprlnks_std 
                                 + v2psorgs_std + v2psprbrch_std)/6)

ggplot(data = temp, aes(x = ps, y = ps_new))+
  geom_point() + ## almost identical
  labs(title   = "calculated party strength index",
       x       = "calculated party strength index",
       y       = "original party strength index",
       caption = "Caluclated party strength index is derived from data after 1999.")
...
data_VD_2000 %>% 
        ...
        geom_point()+
        labs(title   = "Party Strength Index with 5 variables after 1999",
             x       = "Original Party Strength Index (with 6 variables)",
             y       = "Party Strength Index with 5 variables",
             caption = "Party strength Index is derived from data after 1999.")
...
data_merged <-data_EGOV %>% 
              ...
              rename("year" = Survey.Year)                           ## rename variables
...
```{r libdem, include = FALSE}
# checking correlation
data_merged %>% 
  filter(Survey.Year == 2020) %>% 
  ggplot(aes(x = ps_2020, y = v2x_libdem))+
  geom_point()

data_merged %>% 
  filter(Survey.Year == 2020) %>%
  filter(region == "Europe") %>% 
  select(ps_2020, v2x_libdem, E.Government.Index) %>% 
  na.omit() %>% 
  cor()
```

```{r temp_2, eval=FALSE, include=FALSE}
data_merged %>% 
  filter(region == "Europe") %>% 
  filter(Survey.Year == 2014) %>% 
  ggplot(aes(x = ps, y = E.Government.Index))+
  geom_point()

data_merged %>% 
  filter(region == "Europe") %>% 
  filter(Survey.Year == 2014) %>% 
  ggplot(aes(x = v2x_libdem, y = E.Government.Index))+
  geom_point()
